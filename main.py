...
def calculate_instantaneous_speed(position_data, time_data, frame):
    ...
    if position_frame_data.empty or time_frame_data.empty:
        return None, None, None  # 返回空值以便显示错误
...
def calculate_average_speed(position_data, time_data, start_frame, end_frame):
    ...
    for frame in range(start_frame, end_frame + 1):
        speed, x, y, z = calculate_instantaneous_speed(position_data, time_data, frame)
    ...
    avg_speed = total_speed / count if count > 0 else None
    avg_x = total_x / count if count > 0 else None
    avg_y = total_y / count if count > 0 else None
    avg_z = total_z / count if count > 0 else None
    
    return avg_speed, avg_x, avg_y, avg_z
...
def main():
    ...
    if position_data is not None and time_data is not None:
        ...
        if st.button("👅你真棒！终于计算出了瞬时速度💖~"):
            instantaneous_speed, x, y, z = calculate_instantaneous_speed(position_data, time_data, frame)
            if instantaneous_speed is not None:
                st.write(f"帧 {frame} 的瞬时速度为: {instantaneous_speed:.6f} 米/秒")
                st.write(f"帧 {frame} 的位置：X={x}, Y={y}, Z={z}")
            else:
                st.write("该帧的数据不存在。")
        ...
        if st.button("😃计算选定帧范围的平均速度与位移🧮"):
            if start_frame <= end_frame:
                avg_speed, avg_x, avg_y, avg_z = calculate_average_speed(position_data, time_data, start_frame, end_frame)
                ...
                if avg_speed is not None and displacement is not None:
                    ...
                    st.write(f"平均位置：X={avg_x}, Y={avg_y}, Z={avg_z}")
